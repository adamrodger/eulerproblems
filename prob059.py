...
f = open("Data/prob059.txt", "r")
encrypted = f.readline().replace("\n", "").split(",")

for password in permutations(range(97, 123), 3):
    decrypted = [int(c) ^ password[i%3] for i, c in enumerate(encrypted)]
    if "".join([chr(c) for c in decrypted]).count(" and ") > 0:
        print "".join([chr(c) for c in password]), sum(decrypted)
        break